...
const localLegalData = {
  ...legalData,
  "Odisha": {
    districts: "Angul.Balangir.Balasore.Bargarh.Bhadrak.Cuttack.Deogarh.Dhenkanal.Gajapati.Ganjam.Jagatsinghpur.Jajpur.Jharsuguda.Kalahandi.Kandhamal.Kendrapara.Keonjhar.Khordha.Koraput.Malkangiri.Mayurbhanj.Nabarangpur.Nayagarh.Nuapada.Puri.Rayagada.Sambalpur.Subarnapur.Sundargarh".split("."),
    practiceAreas: ["Criminal Law", "Corporate Law", "Civil Law", "Cyber Law", "Family Law"]
  }
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`flex items-center gap-3 rounded-xl px-4 border ${isLight ? "bg-slate-100 border-slate-300" : "bg-slate-950 border-cyan-500/20"}`}>
          ...
          >
            ...
            {Object.keys(localLegalData).map((item) => (
        ...
        <div className={`flex items-center gap-3 rounded-xl px-4 border ${isLight ? "bg-slate-100 border-slate-300" : "bg-slate-950 border-cyan-500/20"}`}>
          ...
          >
            ...
            {state && localLegalData[state]?.districts?.map((item) => (
              <option key={item} value={item}>
                {item}
              </option>
            ))}
        ...
        <div className={`flex items-center gap-3 rounded-xl px-4 border ${isLight ? "bg-slate-100 border-slate-300" : "bg-slate-950 border-cyan-500/20"}`}>
          ...
          >
            ...
            {state && localLegalData[state]?.practiceAreas?.map((item) => (
              <option key={item} value={item}>
                {item}
              </option>
            ))}
